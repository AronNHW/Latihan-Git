<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>@yield('title', 'Dashboard HMTI')</title>
  @vite(['resources/css/app.css', 'resources/js/app.js'])
  <style>
    /* Styles are copied from aspirasi.blade.php for consistency */
    .dashboard-body { background-color: #f8fafc; }
    .sidebar { /* ... sidebar styles ... */ }
    .main-content { /* ... main content styles ... */ }
    .aspirasi-table { background-color: #ffffff; padding: 24px; border-radius: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); overflow-x: auto; }
    .aspirasi-table table { width: 100%; border-collapse: collapse; margin-top: 16px; }
    .aspirasi-table th, .aspirasi-table td { padding: 12px 16px; text-align: left; border-bottom: 1px solid #eef2ff; vertical-align: middle; }
    .aspirasi-table th { background-color: #f8fafc; font-weight: 700; font-size: 14px; color: #475569; text-transform: uppercase; }
    .aspirasi-table tbody tr:hover { background-color: #f1f5f9; }
    .btn-sm { font-size: 13px; padding: 4px 8px; }
    .btn-info { background-color: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer; text-decoration: none; display: inline-block; }
    .btn-primary { background-color: #f97316; color: white; border: none; border-radius: 6px; cursor: pointer; text-decoration: none; display: inline-block; }
    .btn-success { background-color: #22c55e; color: white; border: none; border-radius: 6px; cursor: pointer; text-decoration: none; display: inline-block; }
    .btn-danger { background-color: #ef4444; color: white; border: none; border-radius: 6px; cursor: pointer; }
    .form-control { width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 6px; }
    .form-group { margin-bottom: 1rem; }
  </style>
</head>
<body class="dashboard-body">

  <aside class="sidebar">
    <div class="sidebar-header">
      <img src="{{ asset('images/Logo_hima.png') }}" alt="Logo HMTI" class="sidebar-logo">
      <span class="sidebar-title">HMTI POLITALA</span>
    </div>
    <nav class="sidebar-nav">
      <a href="{{ route('dashboard') }}" class="nav-item {{ request()->routeIs('dashboard') ? 'active' : '' }}">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="nav-icon"><path d="M4 13h6v-2H4v2zm0 4h6v-2H4v2zm0-8h6V7H4v2zm8 2h12V7H12v2zm0 4h12v-2H12v2zm0 4h12v-2H12v2z"></path></svg>
        <span>Dashboard</span>
      </a>
      <a href="{{ route('aspirations.index') }}" class="nav-item {{ request()->routeIs('aspirations.index') ? 'active' : '' }}">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="nav-icon"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-2 12H6v-2h12v2zm0-3H6V9h12v2zm0-3H6V6h12v2z"></path></svg>
        <span>Aspirasi Masuk</span>
      </a>
      <a href="{{ route('news.manage') }}" class="nav-item {{ request()->routeIs('news.*') || request()->routeIs('news.manage') ? 'active' : '' }}">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="nav-icon"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z"></path></svg>
        <span>Manajemen Berita</span>
      </a>
      <a href="#" class="nav-item">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="nav-icon"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"></path></svg>
        <span>Data Pendaftar</span>
      </a>
      <a href="#" class="nav-item">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="nav-icon"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg>
        <span>Prestasi Mahasiswa</span>
      </a>
      <a href="#" class="nav-item">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="nav-icon"><path d="M12 3L2 12h3v8h14v-8h3L12 3zm5 15h-2v-6h-2v6H9v-6H7v6H5v-8.59l7-7 7 7V18z"></path></svg>
        <span>Manajemen Divisi</span>
      </a>
      <a href="#" class="nav-item">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="nav-icon"><path d="M10 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"></path></svg>
        <span>Profil Himpunan</span>
      </a>
    </nav>
    <div class="sidebar-footer">
      <a href="#" class="nav-item"><span>Logout</span></a>
    </div>
  </aside>

  <main class="main-content">
    @yield('content')
  </main>

</body>
</html>
