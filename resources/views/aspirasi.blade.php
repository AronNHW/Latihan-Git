...
<head>
  ...
  <title>Aspirasi Masuk - HMTI</title>
  {{-- Vite will automatically handle the CSS link --}}
  @vite(['resources/css/app.css', 'resources/js/app.js'])

  <style>
    /* Aspirasi Table Styles */
    .aspirasi-table {
    ...
    .aspirasi-table tbody tr:hover {
      background-color: #f1f5f9;
    }
    .aspirasi-table td[colspan="6"] {
      text-align: center;
      padding: 40px;
      font-size: 16px;
      color: #6b7280;
...
<body class="dashboard-body">
  ...
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a href="{{ route('aspirations.index') }}" class="nav-item active">
      ...
      <a href="{{ route('manajemen-berita') }}" class="nav-item">
      ...
      <table>
        ...
        <tbody>
          @forelse ($aspirations as $aspiration)
            <tr>
              <td>{{ $aspiration->nama_pengirim }}</td>
              <td>{{ $aspiration->email_pengirim }}</td>
              <td>{{ $aspiration->subjek }}</td>
              <td>{{ $aspiration->pesan }}</td>
              <td>{{ $aspiration->created_at->format('d-m-Y H:i') }}</td>
              <td>
                <form action="{{ route('aspirations.destroy', $aspiration->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus aspirasi ini?');">
                  @csrf
                  @method('DELETE')
          ...
          @empty
            <tr>
              <td colspan="6">Tidak ada aspirasi yang masuk.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
